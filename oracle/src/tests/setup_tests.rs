...
use crate::price_oracle::{PriceOracleContractBase, PriceOracleContractBaseClient};
use crate::types::{Asset, ConfigData, FeeConfig, PriceUpdate};
...
pub(super) fn init_contract<'a>() -> (Env, PriceOracleContractBaseClient<'a>, ConfigData) {
    let env = Env::default();
    ...
    env.register_at(contract_id, PriceOracleContractBase, ());
    let client = PriceOracleContractBaseClient::new(&env, contract_id);

    env.cost_estimate().budget().reset_unlimited();

    env.mock_all_auths();
    let init_data = prepare_contract_config(&env);
    let initial_expiration_period = 180u32;
    client.config(&init_data, &initial_expiration_period);
...
pub(super) fn generate_update_record_mask(e: &Env, updates: &Vec<i128>) -> Bytes {
    ...
    for (asset_index, price) in updates.iter().enumerate() {
        if price > 0 {
            let (byte, bitmask) =
                crate::mapping::resolve_period_update_mask_position(asset_index as u32);
            let i = byte as usize;
